// app/(_service)/components/nav-bar/admin-flow/editable-nav-bar-content.tsx
"use client";

/**
 * Extracted presentational/content component to keep wrapper small.
 * Uses the admin provider hook useAdminNavigationMenu.
 */
...
export default function EditableNavBarContent() {
  ...
  const getButtonVariant = () => {
    if (!isAdminPagesRoute) return "destructive";
    if (dirty) return "default";
    return "default";
  };

  const getButtonClassName = () => {
    if (!isAdminPagesRoute) return "";
    if (dirty) return "bg-orange-600 hover:bg-orange-700 text-white";
    return "bg-green-600 hover:bg-green-700 text-white";
  };

  const getButtonText = () => {
    if (!isAdminPagesRoute) return isOpen ? "Close bar menu" : "Open bar menu";
    if (isSaving) return "Uploading Changes";
    if (dirty) return "Save and Return";
    return "Return";
  };

  const getButtonIcon = () => {
    if (isSaving) return <Loader2 className="w-4 h-4 animate-spin" />;
    if (isAdminPagesRoute) return <ArrowLeft className="w-4 h-4" />;
    if (isLargeScreen) {
      return (
        <ChevronDown
  ...
  return (
    <DialogsProvider>
      ...
      {isOpen && (
        <div
          className="absolute inset-0 bg-black/50 backdrop-blur-sm transition-opacity duration-300 ease-in-out z-40"
          style={{ top: "64px" }}
          onClick={closeModal}
          aria-hidden="true"
        />
